...
document
  ...
  .addEventListener("change", calculateSha256);
...
async function uploadFilelarge(formData) {
  ...
  console.log("upload  large file by url");
  const analysisResponse = await fetch(uploadUrl, {
    method: "POST",
    headers: {
      "x-apikey": API_KEY,
    },
...
async function mainfun() {
  ...
  document.getElementById("input-div").replaceWith(loader);
  document.getElementById("fileSizeWarning").hidden = true;
  try {
    if (fileSizeInBytes >= 33554432 && fileSizeInBytes <= 209715200) {
      let res = await uploadFilelarge(formData);
    } else if (fileSizeInBytes <= 33554432) {
    ...
    if((Object.keys(brifreport).length)==0){
      throw new Error("no analysis results found try again....")
    }
  ...
  `;
   ...
   let rld= document.createElement("div");
   rld.innerHTML=`<button class="reload-button" onclick=relobtn()>
  ...
  <span class="lable">Reload</span>
    ...
    document.getElementById("navbar").append(rld);
  } catch (error) {
  ...
  <span class="lable">Reload</span>
    ...
    `;
    document.getElementById("parent").replaceWith(err);
  }
}
function relobtn() {
  location.reload();
